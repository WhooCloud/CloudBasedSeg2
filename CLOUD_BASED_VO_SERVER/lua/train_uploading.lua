...
local root_path = "/home/yun/git_repo/CloudBasedSeg2/CLOUD_BASED_VO_SERVER/"
    ...
    while true do
        ...
        if typ == "header" then
            ...
            if robot_id then
                ...
                if file_name then
                    local file_type = string.sub(file_name, #file_name-2, #file_name)
                    if file_type == "mat" then
                        file,err = io.open(dst_dir .. '/' .. robot_id .. '/cls/' .. file_name, "w+")
                        if not file then
                            ngx.say("failed to open file: ", dst_dir .. '/' .. robot_id .. '/cls/' .. file_name)
                            ngx.say(err)
                            return
                        end
                    elseif file_type == "txt" then
                        ...
                        file,err = io.open(dst_dir .. '/' .. robot_id .. '/img/' .. file_name, "w+")
                        if not file then
                            ngx.say("failed to open file: ", dst_dir .. '/' .. robot_id .. '/img/' .. file_name)
                            ngx.say(err)
                            return
                        end                         
                    end
