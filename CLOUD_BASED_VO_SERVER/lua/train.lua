...
local function modifyPrototxt(protofile, robot_id)
  local fp = io.open(protofile, 'r')
  local content = fp:read('*a')
  fp:close()
  content = string.gsub(content, "ROBOTID", robot_id)
  fp = io.open(protofile, 'w')
  fp:write(content)
  fp:close()
end
...
local python_path = root_path .. "../caffe-future/server_train.py"
while true do
  --Initial Message
    ...
    if json_data.type == "initial" then
      ...
      if directory_exists(robot_path) then
          ...
          if not cmd_ok then
            ...
            os.execute("sleep 1")
            modifyPrototxt(robot_path.."solver.prototxt", robot_id)
            modifyPrototxt(robot_path.."train.prototxt", robot_id)
	    result.message = "OK"
          end
      end
    elseif json_data.type == "command" then
      local iter = json_data.iter
      ngx.log(ngx.INFO, "robot_id: ", robot_id)
      ngx.log(ngx.INFO, "iter: ", iter)
      os.execute("python ".. python_path .. " -r "..robot_id.." -i "..iter)
      result.message = "OK"
    else
